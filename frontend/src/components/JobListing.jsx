...
const JobListing = () => {
  ...
  // Filter jobs whenever filters or search change
  useEffect(() => {
  ...
  return (
    <div className="container 2xl:px-20 mx-auto flex flex-col lg:flex-row max-lg:space-y-8 py-8">
      ...
      {/* Job listing */}
      <section className="w-full lg:w-3/4 text-brown-700 max-lg:px-4">
        ...
        {filteredJobs.length > 0 && (
          <div className="flex items-center justify-center space-x-2 mt-10">
            ...
            {Array.from({ length: Math.ceil(filteredJobs.length / 6) }).map((_, pageIndex) => (
              <a key={pageIndex} href="#job-list">
                <button
                  onClick={() => setCurrentPage(pageIndex + 1)}
                  className={`w-10 h-10 flex items-center justify-center border border-gray-300 rounded ${
                    currentPage === pageIndex + 1 ? 'bg-blue-100 text-blue-500' : 'text-gray-500'
                  }`}
                >
                  {pageIndex + 1}
                </button>
